...
def getLastQ(user):
    ...
    cur = cnx.cursor()
    cur.execute(
        f"select l.lastQ from lastQ l inner join clients c on l.usr = c.id where c.phone like {user} or c.fullname like {user};")
    lastQ = cur.fetchone()
    cnx.close()
    return lastQ
...
class WhatsApp:
    ...
    def send_to(self, user, msg):
        ...
        msg += f"""\n\n# --> {"Page D'Accueil" if lng == "fr" else "الصفحة الرئيسية"}\n* --> {"Change Language" if lng == "fr" else "تغير اللغة"}\n## --> {"Étape précédente" if lng == "fr" else "الخطوة السابقة"} """
        if "\n" in msg:
...
tr = {
    "merci pour votre temps, un agent continuera avec vous dès que possible": "شكرًا لك على وقتك ، سيتواصل معك أحد الوكلاء في أقرب وقت ممكن"}

QnA = {
    "Quel sont les profils pour lesquels vous recrutez ?": "Restauration et Hôtellerie ( Managers, Shift-managers, Cuisiniers et Pâtissiers, Boulangers). Si vous êtes intéressés, envoyez-nous vos cordonnés pour qu'on puisse vous contacter. Juste votre numéro de tél suffit",
